...
# Одна и та же аудитория в msu.tj пишется по-разному. Сводим к тому имени,
# которое стоит в CANONICAL_ROOMS, иначе занятость просто теряется:
# 2 сентября 2026 физкультура 19 раз стояла в «стд», а «стадион» из
# канонического списка не встречался ни разу — и «Свободные аудитории»
# уверенно показывали стадион свободным, пока там шла пара.
ROOM_ALIASES = {
    "стд": "стадион",
    "стад": "стадион",
    "лаб.хим": "лабхим",
...
def parse_room(cell_value) -> Optional[str]:
    ...
    аудиторией (иначе в БД появляются дубли), и разворачиваем сокращения
    по ROOM_ALIASES."""
    ...
    if len(parts) >= 2 and len(parts) % 2 == 0:
        ...
        if parts[:half] == parts[half:]:
            val = " ".join(parts[:half])
    # Сокращения — по каждому слову: в ячейке может стоять «стд 107».
    val = " ".join(ROOM_ALIASES.get(tok, tok) for tok in val.split())
    return val
