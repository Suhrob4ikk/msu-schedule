...
export default function NotificationToggle() {
  const [status, setStatus] = useState<"loading" | "unsupported" | "denied" | "off" | "on">(() => {
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors min-h-[44px] ${status === "on"
    ...
    >
      {status === "on" ? (
        <>
          ...
          Уведомления о зачётах / экзаменах вкл.
        </>
      ) : status === "denied" ? (
        <>
          <svg className="w-4 h-4 shrink-0" fill="currentColor" viewBox="0 0 20 20">
